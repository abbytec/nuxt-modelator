...
const doUpdateStock = async () => {
	...
	await store.updateStock({ id: stockForm.value.id, data: { $inc: { stock: Number(stockForm.value.amount) || 1 } } });
};

const doDelete = async () => {
	if (!deleteId.value) return;
	await store.delete({ id: deleteId.value });
};
				...
				<div class="space-y-3">
					...
					<div class="flex gap-2">
						<BaseButton
							...
							:disabled="store.loading.saveOrUpdate"
							@click="doUpsert">
