...
const createSampleProduct = async () => {
	...
	try {
		await store.create({ ...newProduct.value, slug: newProduct.value.name });
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
			<div class="bg-green-50 p-6 rounded-lg">
				...
				<BaseButton
					...
					class="w-full mt-4 bg-green-600 text-white hover:bg-green-700 disabled:opacity-50">
			...
			<div class="bg-red-50 p-6 rounded-lg">
				...
				<BaseButton
					...
					:disabled="store.loading.create"
					class="w-full bg-red-600 text-white hover:bg-red-700 disabled:opacity-50">
					❌ Probar Validaciones
				</BaseButton>
			</div>
		</div>
