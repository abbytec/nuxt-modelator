	...
	if (!modelMeta.props.find((prop) => prop.name === "id" || prop.name === "_id")) {
        ...
        for (const prop of modelMeta.props) {
                ...
                for (const transform of prop.transforms) {
                        if (transform.kind === "mongoId") {
                                fieldName = "_id";
                                fieldDef.type = mg.Schema.Types.ObjectId;
                                const autogenerated = transform.config?.autogenerated !== false;
                                fieldDef.required = !autogenerated;
                                fieldDef.auto = autogenerated;
                        }
...
function normalizeFilterId(filter: any): any {
        if (filter && typeof filter === "object") {
                if (filter.id && !filter._id) {
                        const mg = mongoose;
                        try {
                                filter._id = new mg.Types.ObjectId(filter.id);
                        } catch {
...
function prepareDataId(data: any): any {
        if (data && typeof data === "object") {
                if (data.id && !data._id) {
                        const mg = mongoose;
                        try {
                                data._id = new mg.Types.ObjectId(data.id);
                        } catch {
                                data._id = data.id;
                        }
                        delete data.id;
...
function mapMongoId(doc: any): any {
        if (Array.isArray(doc)) return doc.map(mapMongoId);
...
export async function findDocuments<T = any>(
	...
	options: { limit?: number; skip?: number; sort?: any; populate?: string[] } = {}
        ...
        try {
                ...
                filter = normalizeFilterId(filter);
		...
		if (options.populate) {
			options.populate.forEach((field) => {
                ...
                const mapped = mapMongoId(data);

                return { success: true, data: mapped, count: Array.isArray(mapped) ? mapped.length : undefined };
	} catch (error) {
...
export async function createDocument<T = any>(modelMeta: ModelMeta, data: any): Promise<CrudOperationResult<T>> {
        try {
                const model = await getOrCreateMongoModel(modelMeta);
                const prepared = prepareDataId({ ...data });
                const doc = new model(prepared);
                const saved = await doc.save();
                const mapped = mapMongoId(saved);
...
export async function updateDocument<T = any>(
        ...
        try {
                const model = await getOrCreateMongoModel(modelMeta);
                filter = normalizeFilterId(filter);
                const prepared = prepareDataId({ ...update });
                const data = await model.findOneAndUpdate(filter, prepared, options).exec();
                const mapped = mapMongoId(data);
...
export async function saveOrUpdateDocument<T = any>(modelMeta: ModelMeta, data: any, filter?: any): Promise<CrudOperationResult<T>> {
        try {
                ...
                const preparedData = prepareDataId({ ...data });
                const normalizedFilter = normalizeFilterId(filter) || (preparedData._id ? { _id: preparedData._id } : null);

                if (normalizedFilter) {
                        const updated = await model
                                .findOneAndUpdate(normalizedFilter, preparedData, { new: true, upsert: true })
                                .exec();
                        const mapped = mapMongoId(updated);
                        return { success: true, data: mapped };
                } else {
                        const doc = new model(preparedData);
                        const saved = await doc.save();
                        const mapped = mapMongoId(saved);
                        return { success: true, data: mapped };
                }
