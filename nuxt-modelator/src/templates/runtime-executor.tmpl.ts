...
import { serverMiddlewares, defaultReturn } from '#nuxt-modelator/runtime/middlewares'

type MWSpec = string | { name: string; args?: any; stage?: 'server'|'client'|'isomorphic' }
...
async function runPipeline(event: H3Event, model: string, op: string, specs: MWSpec[], customReturnName?: string, modelMeta?: any) {
  ...
  for (const spec of specs) {
    ...
    if (stage !== 'server') continue // sólo server en handlers
    const factory = serverMiddlewares[name]
